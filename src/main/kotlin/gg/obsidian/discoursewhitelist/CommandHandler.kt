...
class CommandHandler(val plugin: Plugin): CommandExecutor {
    ...
    override fun onCommand(player: CommandSender, cmd: Command, alias: String?, args: Array<out String>?): Boolean {
        ...
        if (cmd.name != "discoursewhitelist") return true

        if (args == null || args.size != 1 || !args[0].equals("reload")) {
            sendMessage("&eUsage: /discoursewhitelist reload", player, isConsole)
            return true
        }

        sendMessage("&eReloading Discourse Whitelist configuration...", player, isConsole)
        plugin.reload()
        return true
    }
